...
export async function POST(req: NextRequest) {
  try {
    ...
    if (USE_MAILGUN) {
      ...
      const mgList = process.env.MAILGUN_LIST // optional: a mailing list like newsletter@your.domain
      if (!mgApiKey || !mgDomain) {
        return NextResponse.json({ error: 'Mailgun not configured.' }, { status: 500 })
      }

      // Prefer adding to a Mailgun mailing list if provided; otherwise, store as a tag via events
      if (mgList) {
        ...
        // Fallback: send a confirmation email via Mailgun Messages API
      ...
      // Google Workspace via SMTP (Nodemailer)
